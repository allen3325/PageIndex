...
async def generate_summaries_for_structure_md(structure, summary_token_threshold, model=None, opt=None):
    ...
    if opt and not getattr(opt, 'parallel_requests', True):
        ...
        summaries = []
        for node in nodes:
            summary = await get_node_summary(node, summary_token_threshold=summary_token_threshold, model=model, opt=opt)
        ...
        async def limited_task(node):
            async with semaphore:
                return await get_node_summary(node, summary_token_threshold=summary_token_threshold, model=model, opt=opt)
